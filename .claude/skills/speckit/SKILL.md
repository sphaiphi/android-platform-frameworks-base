...
| File | Purpose |
|---|---|
| `.specify/workflows/speckit-full/workflow.yml` | Full pipeline: constitution → specify → clarify → plan → checklist → analyze → tasks → implement → evolve |
| `.specify/workflows/speckit-evolve/workflow.yml` | Single-task OpenEvolve optimization: prepare → gate → integrate → verify |
...
**Subagent:** When running the constitution step yourself (outside Claude Code), spawn the
`agents/speckit-constitution.md` subagent with these parameters:
...
**Subagent:** When running the specify step yourself (outside Claude Code), spawn the
`agents/speckit-specify.md` subagent with these parameters:
...
**Subagent:** When running the clarify step yourself (outside Claude Code), spawn the
`agents/speckit-clarify.md` subagent with these parameters:
...
**Subagent:** When running the plan step yourself (outside Claude Code), spawn the
`agents/speckit-plan.md` subagent with these parameters:
- `spec_path`: `.specify/specs/<feature-id>/spec.md`
- `constitution_path`: `.specify/memory/constitution.md` (if it exists)
...
## Step 5: Analyze — `/speckit.analyze` (optional but recommended)
...
**Subagent:** When running the analyze step yourself (outside Claude Code), spawn the
`agents/speckit-analyze.md` subagent with these parameters:
- `spec_path`: `.specify/specs/<feature-id>/spec.md`
...
**Subagent:** When running the tasks step yourself (outside Claude Code), spawn the
`agents/speckit-tasks.md` subagent with these parameters:
- `plan_path`: `.specify/specs/<feature-id>/plan.md`
- `spec_path`: `.specify/specs/<feature-id>/spec.md`
- `data_model_path`: `.specify/specs/<feature-id>/data-model.md`
- `contracts_dir`: `.specify/specs/<feature-id>/contracts/`
...
## Step 6.5: Thinking Design — `/speckit.thinking`
...
**Subagent:** `agents/speckit-thinking.md`
...
**Subagent:** When running the implement step yourself (outside Claude Code), spawn the
`agents/speckit-implement.md` subagent with these parameters:
- `tasks_path`: `.specify/specs/<feature-id>/tasks.md`
- `plan_path`: `.specify/specs/<feature-id>/plan.md`
- `spec_path`: `.specify/specs/<feature-id>/spec.md`
- `data_model_path`: `.specify/specs/<feature-id>/data-model.md`
...
| `agents/speckit-thinking.md` | Thinking | Implementation design: typed component interfaces, data flow pipelines, interface contracts, file structure, behavioural scenarios, dependency graph, trade-offs |
...
```bash
specify workflow add .claude/skills/spec-kit/workflows/speckit-full.yml
specify workflow add .claude/skills/spec-kit/workflows/speckit-evolve.yml
...
## OpenEvolve Integration — `/speckit.evolve`
...
**When to use `/speckit.evolve`:** After `speckit.implement` has built the structural scaffold, for any task that scored 4–5 on the Evolution Fitness Test.
...
**Running OpenEvolve with Claude as the LLM backend:**

OpenEvolve requires an OpenAI-compatible API. Use LiteLLM as a proxy:

```bash
pip install litellm[proxy]
litellm --model claude-sonnet-4-6 --port 8000

# Then set:
export OPENAI_API_KEY="sk-ant-..."  # your Anthropic key
```

The generated `config.yaml` points to `http://localhost:8000/v1` by default.
