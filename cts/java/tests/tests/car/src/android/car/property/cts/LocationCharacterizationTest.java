...
public class LocationCharacterizationTest {
    ...
    public void testToString() {
        ...
        assertThat(
                        LocationCharacterization.toString(
    ...
    public void testAllLocationCharacterizationsAreMappedInToString() {
        ...
        for (Integer locationCharacterization : locationCharacterizations) {
            String automaticEmergencyBrakingString =
                    LocationCharacterization.toString(locationCharacterization);
            assertWithMessage("%s starts with 0x", automaticEmergencyBrakingString)
                    .that(automaticEmergencyBrakingString.startsWith("0x"))
                    .isFalse();
        }
    }
    ...
    public void testNoLocationCharacterizationsOverlap() {
        ...
        for (Integer locationCharacterization : locationCharacterizations) {
            assertWithMessage(
                            "LocationCharacterization values must not bit-wise overlap. "
                                    + "Found value: "
                                    + locationCharacterization
                                    + ", whose bit values already "
