...
public class ForwardCollisionWarningStateTest {
    ...
    public void testAllForwardCollisionWarningStatesAreMappedInToString() {
        List<Integer> forwardCollisionWarningStates =
                VehiclePropertyUtils.getIntegersFromDataEnums(ForwardCollisionWarningState.class);
        for (Integer forwardCollisionWarningState : forwardCollisionWarningStates) {
            String forwardCollisionWarningStateString =
                    ForwardCollisionWarningState.toString(forwardCollisionWarningState);
            assertWithMessage("%s starts with 0x", forwardCollisionWarningStateString)
                    .that(forwardCollisionWarningStateString.startsWith("0x"))
                    .isFalse();
        }
    }
}
