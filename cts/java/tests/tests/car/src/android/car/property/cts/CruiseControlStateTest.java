...
public class CruiseControlStateTest {
    ...
    public void testToString() {
        assertThat(CruiseControlState.toString(CruiseControlState.OTHER)).isEqualTo("OTHER");
        assertThat(CruiseControlState.toString(CruiseControlState.ENABLED)).isEqualTo("ENABLED");
        ...
        assertThat(CruiseControlState.toString(6)).isEqualTo("0x6");
        assertThat(CruiseControlState.toString(12)).isEqualTo("0xc");
    }

    @Test
    public void testAllCruiseControlStatesAreMappedInToString() {
        List<Integer> cruiseControlStates =
                VehiclePropertyUtils.getIntegersFromDataEnums(CruiseControlState.class);
        for (Integer cruiseControlState : cruiseControlStates) {
            String cruiseControlStateString = CruiseControlState.toString(cruiseControlState);
            assertWithMessage("string for mode %s must not start with 0x", cruiseControlStateString)
                    .that(cruiseControlStateString.startsWith("0x"))
                    .isFalse();
        }
    }
}
