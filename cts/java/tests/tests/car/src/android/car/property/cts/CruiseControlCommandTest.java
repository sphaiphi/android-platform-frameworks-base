...
public class CruiseControlCommandTest {
    ...
    public void testToString() {
        ...
        assertThat(CruiseControlCommand.toString(7)).isEqualTo("0x7");
        assertThat(CruiseControlCommand.toString(12)).isEqualTo("0xc");
    }

    @Test
    public void testAllCruiseControlCommandsAreMappedInToString() {
        List<Integer> cruiseControlCommands =
                VehiclePropertyUtils.getIntegersFromDataEnums(CruiseControlCommand.class);
        for (Integer cruiseControlCommand : cruiseControlCommands) {
            String cruiseControlCommandString = CruiseControlCommand.toString(cruiseControlCommand);
            assertWithMessage(
                            "string for mode %s must not start with 0x", cruiseControlCommandString)
                    .that(cruiseControlCommandString.startsWith("0x"))
                    .isFalse();
        }
    }
}
