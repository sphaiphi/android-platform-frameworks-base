...
public class TrailerStateTest {
    ...
    public void testToString() {
        assertThat(TrailerState.toString(TrailerState.STATE_UNKNOWN)).isEqualTo("STATE_UNKNOWN");
        assertThat(TrailerState.toString(TrailerState.STATE_NOT_PRESENT))
                .isEqualTo("STATE_NOT_PRESENT");
        assertThat(TrailerState.toString(TrailerState.STATE_PRESENT)).isEqualTo("STATE_PRESENT");
        assertThat(TrailerState.toString(TrailerState.STATE_ERROR)).isEqualTo("STATE_ERROR");
    ...
    public void testAllTrailerStatesAreMappedInToString() {
        List<Integer> trailerStates =
                VehiclePropertyUtils.getIntegersFromDataEnums(TrailerState.class);
        for (Integer trailerState : trailerStates) {
            String trailerStateString = TrailerState.toString(trailerState);
            assertWithMessage("%s starts with 0x", trailerStateString)
                    .that(trailerStateString.startsWith("0x"))
                    .isFalse();
        }
    }
}
