...
public final class EvChargeStateTest extends AbstractCarLessTestCase {
    ...
    public void testToString() {
        ...
        assertThat(EvChargeState.toString(EvChargeState.STATE_CHARGING))
                .isEqualTo("STATE_CHARGING");
        assertThat(EvChargeState.toString(EvChargeState.STATE_FULLY_CHARGED))
                .isEqualTo("STATE_FULLY_CHARGED");
        assertThat(EvChargeState.toString(EvChargeState.STATE_NOT_CHARGING))
                .isEqualTo("STATE_NOT_CHARGING");
    ...
    public void testAllEvChargeStatesAreMappedInToString() {
        List<Integer> evChargeStates =
                VehiclePropertyUtils.getIntegersFromDataEnums(EvChargeState.class);
        for (Integer evChargeState : evChargeStates) {
            String evChargeStateString = EvChargeState.toString(evChargeState);
            assertWithMessage("%s starts with 0x", evChargeStateString)
                    .that(evChargeStateString.startsWith("0x"))
                    .isFalse();
        }
    }
}
