...
public class VehicleOilLevelTest {
    ...
    public void testToString() {
        ...
        assertThat(VehicleOilLevel.toString(VehicleOilLevel.LEVEL_LOW)).isEqualTo("LEVEL_LOW");
        assertThat(VehicleOilLevel.toString(VehicleOilLevel.LEVEL_NORMAL))
                .isEqualTo("LEVEL_NORMAL");
        assertThat(VehicleOilLevel.toString(VehicleOilLevel.LEVEL_HIGH)).isEqualTo("LEVEL_HIGH");
        assertThat(VehicleOilLevel.toString(VehicleOilLevel.LEVEL_ERROR)).isEqualTo("LEVEL_ERROR");
    ...
    public void testAllVehicleOilLevelsAreMappedInToString() {
        List<Integer> vehicleOilLevels =
                VehiclePropertyUtils.getIntegersFromDataEnums(VehicleOilLevel.class);
        for (Integer vehicleOilLevel : vehicleOilLevels) {
            String vehicleOilLevelString = VehicleOilLevel.toString(vehicleOilLevel);
            assertWithMessage("%s starts with 0x", vehicleOilLevelString)
                    .that(vehicleOilLevelString.startsWith("0x"))
                    .isFalse();
        }
    }
}
