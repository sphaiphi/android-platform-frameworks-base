...
public class LaneCenteringAssistCommandTest {
    ...
    public void testAllLaneCenteringAssistCommandsAreMappedInToString() {
        List<Integer> laneCenteringAssistCommands =
                VehiclePropertyUtils.getIntegersFromDataEnums(LaneCenteringAssistCommand.class);
        for (Integer laneCenteringAssistCommand : laneCenteringAssistCommands) {
            String laneCenteringAssistCommandString =
                    LaneCenteringAssistCommand.toString(laneCenteringAssistCommand);
            assertWithMessage("%s starts with 0x", laneCenteringAssistCommandString)
                    .that(laneCenteringAssistCommandString.startsWith("0x"))
                    .isFalse();
        }
    }
}
