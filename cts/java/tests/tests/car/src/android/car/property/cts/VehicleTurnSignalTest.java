...
public final class VehicleTurnSignalTest {
    ...
    public void testAllVehicleTurnSignalsAreMappedInToString() {
        List<Integer> vehicleTurnSignals =
                VehiclePropertyUtils.getIntegersFromDataEnums(VehicleTurnSignal.class);
        for (Integer vehicleTurnSignal : vehicleTurnSignals) {
            ...
            assertWithMessage("%s starts with 0x", vehicleTurnSignalString)
                    .that(vehicleTurnSignalString.startsWith("0x"))
                    .isFalse();
        }
    }
}
