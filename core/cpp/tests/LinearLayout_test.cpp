...
TEST(LinearLayoutTest, WeightDistribution) {
    auto layout = std::make_shared<LinearLayout>();
    layout->set_orientation(LinearLayout::VERTICAL);
    
    auto child1 = std::make_shared<View>();
    child1->set_layout_params(std::make_shared<LinearLayout::LayoutParams>(100, 0, 1.0f));
    layout->add_view(child1);
    ...
    EXPECT_EQ(0, child1->get_top());
    EXPECT_EQ(100, child1->get_bottom());
    
    EXPECT_EQ(100, child2->get_top());
    EXPECT_EQ(200, child2->get_bottom());
}

TEST(LinearLayoutTest, GravityAlignment) {
    auto layout = std::make_shared<LinearLayout>();
    layout->set_orientation(LinearLayout::VERTICAL);
    
    auto child = std::make_shared<View>();
    auto lp = std::make_shared<LinearLayout::LayoutParams>(50, 50);
